...
import { binaryStringToArray } from '../../lib/utils';
...
test('it verifies a message with multiple signatures', async (t) => {
    const publicKey1 = await openpgp.readKey({ armoredKey: armoredPublicKey });
    const publicKey2 = await openpgp.readKey({ armoredKey: armoredPublicKey2 });
    const { data, verified, signatureTimestamp, signatures, errors } = await verifyMessage({
        message: await createMessage('hello world'),
        signature: await openpgp.readSignature({ armoredSignature: detachedSignatureFromTwoKeys }),
...
test('it does not verify a message given wrong public key', async (t) => {
    const { publicKey: wrongPublicKey } = await openpgp.generateKey({
        userIDs: [{ name: 'test', email: 'a@b.com' }],
        format: 'object'
    });
    const { verified, signatureTimestamp, signatures } = await verifyMessage({
        message: await createMessage('hello world'),
        signature: await openpgp.readSignature({ armoredSignature: detachedSignatureFromTwoKeys }),
