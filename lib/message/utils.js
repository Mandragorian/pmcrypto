...
export async function getMessage(message) {
    if (openpgp.Message.prototype.isPrototypeOf(message)) {
        return message;
    }
    if (Uint8Array.prototype.isPrototypeOf(message)) {
        return openpgp.readMessage({ binaryMessage: message });
    }
    return openpgp.readMessage({ armoredMessage: message.trim() });
}
...
export async function getSignature(signature) {
    if (openpgp.Signature.prototype.isPrototypeOf(signature)) {
        return signature;
    }
    if (Uint8Array.prototype.isPrototypeOf(signature)) {
        return openpgp.readSignature({ binarySignature: signature });
    }
    return openpgp.readSignature({ armoredSignature: signature.trim() });
}
...
export async function getCleartextMessage(message) {
    if (openpgp.CleartextMessage.prototype.isPrototypeOf(message)) {
        return message;
    }
    return openpgp.readCleartextMessage({ armoredMessage: message.trim() });
}
...
export function createCleartextMessage(message) {
    ...
    return openpgp.createCleartextMessage({ text: message });
}

export function createMessage(source, filename, date = serverTime()) {
    if (Uint8Array.prototype.isPrototypeOf(source)) {
        return openpgp.createMessage({ binary: source, filename, date });
    }
    return openpgp.createMessage({ text: source, filename, date });
}
